...
if __name__ == "__main__":
	...
	def get_image_name(n, original=False, thumbnail=False):
		s = f'{prefix}-{str(n).zfill(args.zfill)}'
		if thumbnail: s += f'-thumbnail.{args.suffix}'
		elif original: s += f'.{args.suffix}'
		return s
	...
	if not args.skip_thumbnails:
		...
		for photo in photos:
			...
			thumbnail_filename = f'{photo[:-(len(args.suffix)+1)]}-thumbnail.{args.suffix}'
